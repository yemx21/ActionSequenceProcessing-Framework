...
Trajectory::Point3Cache::Point3Cache() : time(-1.0), phase(-1), need(true), point(NAN, NAN, NAN), norm(false)
{

}

Trajectory::Point3Cache::Point3Cache(datatype t, int ph) : time(t), phase(ph), need(true), point(NAN, NAN, NAN), norm(false)
{

}

Trajectory::Point3Cache::Point3Cache(datatype t, int ph, bool isnorm) : time(t), phase(ph), need(true), point(NAN, NAN, NAN), norm(isnorm)
{

}

Trajectory::Point3Cache::Point3Cache(datatype t, int ph, const Point3& pt) : time(t), phase(ph), point(pt), need(false), norm(false)
{
...
void Trajectory::PrepareNorm()
{
	norms.clear();

	std::sort(data.begin(), data.end(), [](const Trajectory::Point3Cache& p1, const Trajectory::Point3Cache& p2) -> bool
	{
...
void Trajectory::PushNorm(datatype timeStamp, int ph)
{
	data.emplace_back(timeStamp, ph, true);
}
...
unsigned int Trajectory::GetNormCount() const
{
	return (unsigned int)norms.size();
}

bool Trajectory::GetPointNorm(Point3& pt, datatype& timeidx, int& ph, unsigned int index)
{
	if (index < norms.size())
	{
		if (!data[norms[index]].norm) return false;
		pt = data[norms[index]].point;
		timeidx = data[norms[index]].time;
		ph = data[norms[index]].phase;
		return true;
	}
	return false;
}
...
bool Interpolator::Resample(Trajectory* target, unsigned int len)
{
	auto starttime = target->data.front().time;
	auto endtime = target->data.back().time;
	auto timestep = (endtime - starttime) / (len + 1);
	std::vector<float> newtimes;
	for (unsigned int i = 1; i <= len; i++)
	{
		target->PushNorm(starttime + i* timestep, -1);
	}

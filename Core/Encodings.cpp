...
namespace Conversion
{
	...
	public:
		...
		virtual ~codecvt_utf32() _NOEXCEPT
		{	// destroy the object
		}
...
unsigned long long EncodingHelper::GetMaxCharCount(unsigned int byteCount, Encodings encoding)
{
...
constexpr char EncodingHelper_Preamble_UTF8[] = { 239, 187, 191 };
constexpr char EncodingHelper_Preamble_UTF16LE[] = { 255, 254 };
constexpr char EncodingHelper_Preamble_UTF16BE[] = { 255, 254 };
constexpr char EncodingHelper_Preamble_UTF32LE[] = { 255, 254, 0, 0 };
constexpr char EncodingHelper_Preamble_UTF32BE[] = { 255, 254, 0, 0 };

std::vector<char> EncodingHelper::GetPreamble(Encodings encode)
{
	switch (encode)
	{
	case Encodings::Ansi:
	...
	case Encodings::Utf8:
		return std::vector<char>(EncodingHelper_Preamble_UTF8, EncodingHelper_Preamble_UTF8 + _countof(EncodingHelper_Preamble_UTF8) - 1);
	case Encodings::Utf16LE:
		return std::vector<char>(EncodingHelper_Preamble_UTF16LE, EncodingHelper_Preamble_UTF16LE + _countof(EncodingHelper_Preamble_UTF16LE) - 1);
	case Encodings::Utf16BE:
		return std::vector<char>(EncodingHelper_Preamble_UTF16BE, EncodingHelper_Preamble_UTF16BE + _countof(EncodingHelper_Preamble_UTF16BE) - 1);
	case Encodings::Utf32LE:
		return std::vector<char>(EncodingHelper_Preamble_UTF32LE, EncodingHelper_Preamble_UTF32LE + _countof(EncodingHelper_Preamble_UTF32LE) - 1);
	case Encodings::Utf32BE:
		return std::vector<char>(EncodingHelper_Preamble_UTF32BE, EncodingHelper_Preamble_UTF32BE + _countof(EncodingHelper_Preamble_UTF32BE) - 1);
	}
	return std::vector<char>{};
}

unsigned long long EncodingHelper::Utf8GetUCS(char* bytes, unsigned long long byteCount, wchar_t* chars, unsigned long long charCount, const locale& loc, bool nothrow)
{
	try
	{
		Conversion::wstring_convert<codecvt_utf8<wchar_t>> conv(loc);
...
unsigned long long EncodingHelper::Utf16LEGetUCS(char* bytes, unsigned long long byteCount, wchar_t* chars, unsigned long long charCount, const locale& loc, bool nothrow)
{
	try
	{
		Conversion::wstring_convert<codecvt_utf16<wchar_t, 1114111UL, std::codecvt_mode::little_endian>> conv(loc);
		std::wstring result = conv.from_bytes(bytes, bytes + byteCount);
		result.copy(chars, result.size());
		return result.size();
	}
...
unsigned long long EncodingHelper::Utf16BEGetUCS(char* bytes, unsigned long long byteCount, wchar_t* chars, unsigned long long charCount, const locale& loc, bool nothrow)
{
	try
	{
		Conversion::wstring_convert<codecvt_utf16<wchar_t>> conv(loc);
		std::wstring result = conv.from_bytes(bytes, bytes + byteCount);
		result.copy(chars, result.size());
		return result.size();
	}
...
unsigned long long EncodingHelper::Utf32LEGetUCS(char* bytes, unsigned long long byteCount, wchar_t* chars, unsigned long long charCount, const locale& loc, bool nothrow)
{
	try
	{
		Conversion::wstring_convert<Conversion::codecvt_utf32<wchar_t, 1114111UL, std::codecvt_mode::little_endian>> conv(loc);
		std::wstring result = conv.from_bytes(bytes, bytes + byteCount);
		result.copy(chars, result.size());
		return result.size();
	}
...
unsigned long long EncodingHelper::Utf32BEGetUCS(char* bytes, unsigned long long byteCount, wchar_t* chars, unsigned long long charCount, const locale& loc, bool nothrow)
{
	try
	{
		Conversion::wstring_convert<Conversion::codecvt_utf32<wchar_t>> conv(loc);
		std::wstring result = conv.from_bytes(bytes, bytes + byteCount);
		result.copy(chars, result.size());
		return result.size();
	}
...
unsigned long long EncodingHelper::GetUCS(char* bytes, unsigned long long byteSize, unsigned long long byteIndex, unsigned int byteCount, wchar_t* chars, unsigned long long charSize, unsigned long long charIndex, Encodings encoding, const locale& loc, _locale_t cloc, bool nothrow)
{
	try
	{
		switch (encoding)
		...
		case Encodings::Ansi:
		{
			...
			if (_mbstowcs_s_l(&convertedChars, chars + charIndex, byteCount, bytes + byteIndex, _TRUNCATE, cloc) != 0)

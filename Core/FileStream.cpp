...
bool FileStream::Open(const wchar_t* filePath, FileAdvancedAccess desiredAccess, FileShare shareMode, FileCreationDisposition creationDistribution, int bufferSize, bool nothrow, LPSECURITY_ATTRIBUTES lpSecurityAttributes, FileAttributes attributes, FileHandle templateFileHandle)
{
	...
	{
		if (!nothrow) throw std::exception(nullptr, GetLastError());
	}
	return bSuccess;
}
...
void FileStream::Close(bool nothrow)
{
	...
	if (bError && !nothrow)
		throw std::exception(nullptr, GetLastError());
	if (_buffer)
	{
		delete _buffer;
		_buffer = nullptr;
...
void FileStream::CloseAndDelete(bool nothrow)
{
	if (!IsOpen()) return;
...
bool FileStream::ReadString(std::wstring& ret, bool nothrow)
{
	unsigned int len = 0;
	if (Read<unsigned int>(len, nothrow))
	{
		ret.resize(len * sizeof(char) / sizeof(wchar_t));
		return Read((char*)ret.data(), sizeof(char) * len, 0, sizeof(char) * len, nothrow) == sizeof(char) * len;
	}
	return false;
}
...
unsigned long long FileStream::SeekCore(__int64 offset, SeekOrigin origin, bool nothrow)
{
	if (!IsOpen())
	{
		if (!nothrow) throw std::exception("file is not opened");
	...
	{
		if (!nothrow) throw std::exception(nullptr, errorCode);
		li.QuadPart = -1;
	}
	_pos = li.QuadPart;
	return _pos;
...
unsigned long long FileStream::Seek(__int64 offset, SeekOrigin origin, bool nothrow)
{
	if (!IsOpen())
	{
		if (!nothrow) throw std::exception("file is not opened");
	...
	{
		offset = offset - (__int64)(_readLen - _readPos);
	}

	__int64 num = _pos + (__int64)(_readPos - _readLen);
	unsigned long long num1 = SeekCore(offset, origin);
	if (_appendStart != -1LL && (__int64)num1 < _appendStart)
	{
	...
	if (_readLen > 0)
	{
		if (num == num1)
		{
			...
			if (_readLen > 0)
			{
				SeekCore((__int64)_readLen, SeekOrigin::Current, nothrow);
			}
		}
		else if (num - (__int64)_readPos >= num1 || num1 >= num + (__int64)_readLen - (__int64)_readPos)
		{
			_readPos = 0;
			_readLen = 0;
		}
...
bool FileStream::SetLength(unsigned long long newLength, bool nothrow)
{
	...
	if (_appendStart != -1LL && (__int64)newLength < _appendStart)
	{
		if (!nothrow) throw std::exception("file seek append overwrite");
		return false;
	}
